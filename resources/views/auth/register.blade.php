<x-guest-layout>
    <div class="mb-6">
        ...
        <p class="text-sm text-muted-foreground mt-1">Start shipping with your team.</p>
    </div>

    <form method="POST" action="{{ route('register') }}" class="space-y-5">
        @csrf
        <a href="{{ route('auth.google') }}"
            class="w-full flex items-center justify-center gap-2 border border-border rounded-3xl py-2.5 text-sm font-medium text-foreground hover:bg-muted transition mb-5">
        ...
        <div>
            ...
            <x-text-input id="name" class="block mt-1.5 w-full" type="text" name="name" :value="old('name')"
        ...
        <p class="text-center text-sm text-muted-foreground">
            ...
            <a href="{{ route('login') }}" class="text-foreground font-medium hover:underline">{{ __('Log in') }}</a>
        </p>
    </form>
</x-guest-layout>
